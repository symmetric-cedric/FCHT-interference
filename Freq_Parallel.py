#!/usr/bin/env python3
# freq_mpi.py — Numerical-Hessian vibrational analysis with optional MPI parallelism
...
class FreqParallel:
    ...
    def _grad_at_coords(mf, coords_bohr):
        ...
        if getattr(mf, 'with_df', None) is not None:
            aux = getattr(mf.with_df, 'auxbasis', None)
            mf1 = mf1.density_fit(auxbasis=aux)

        mf1.xc = getattr(mf, "xc", "PBE")
        ...
        mf1.kernel()
        g = grad.rks.Gradients(mf1).kernel()
        return g

    def _hessian_cart_numerical(self, mf, step_bohr=0.01, logger_obj=None):
        """
        Finite-difference Hessian. If run under MPI (mpirun/srun), distribute the
        3N columns across ranks. Otherwise, run serially.
        """
        mol = mf.mol
        natm = mol.natm
        n3 = 3 * natm
        ...
        if _SIZE == 1:
            ...
            for i in range(n3):
                A, comp = divmod(i, 3)
                coords_p = coords0.copy(); coords_p[A, comp] += step_bohr
                g_p = self._grad_at_coords(mf, coords_p).reshape(-1)
                coords_m = coords0.copy(); coords_m[A, comp] -= step_bohr
                g_m = self._grad_at_coords(mf, coords_m).reshape(-1)
                H[:, i] = (g_p - g_m) / (2.0 * step_bohr)
            return 0.5 * (H + H.T)

        # -------- Parallel path (MPI) --------
        # Each rank handles a strided subset of columns i = rank, rank+size, ...
        local_cols = []
        for i in range(_RANK, n3, _SIZE):
            A, comp = divmod(i, 3)
            coords_p = coords0.copy(); coords_p[A, comp] += step_bohr
            g_p = self._grad_at_coords(mf, coords_p).reshape(-1)
            coords_m = coords0.copy(); coords_m[A, comp] -= step_bohr
            g_m = self._grad_at_coords(mf, coords_m).reshape(-1)
            col = (g_p - g_m) / (2.0 * step_bohr)
            local_cols.append((i, col.astype(float, copy=False)))

        # Gather all computed columns to rank 0, assemble, then broadcast back
        gathered = _COMM.gather(local_cols, root=0)
        if _RANK == 0:
            H = np.zeros((n3, n3), dtype=float)
    ...
    def _angstrom_to_bohr(self,x_ang):
