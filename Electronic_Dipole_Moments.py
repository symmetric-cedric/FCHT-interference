# TDDFT_HT.py — single-class TDDFT & Herzberg–Teller derivatives
# Minimal inputs when you already have your Freq object.
...
class TDDFTHT:
    ...
    def from_freq(cls, mf, fr, *, istate=0, nstates=5, dq_Q=0.03,
        ...
        """
        Easiest path when you have your Freq object from Parts 1–4.
        Pulls everything needed from `fr` and runs reference + HT immediately.
        """
        tdht = cls(mf,
                   nstates=nstates,
                   tracker_sigma_e_eV=tracker_sigma_e_eV,
                   tracker_w_cos=tracker_w_cos)
        # Store vibrational data from Freq
        tdht.geom_ref = fr.atom_list                 # atom-list (Å) reference geometry
        tdht.l_MWC = np.asarray(fr.l_MWC, float)     # (3N, Nvib)
    ...
    def from_minimal(cls, mf, *, geom_ref_atomlist, l_MWC, evals_int_au, mass_vec_3N,
        ...
        """
        Same idea, but feed minimal arrays directly if you’re not using Freq.
        """
        tdht = cls(mf,
                   nstates=nstates,
                   tracker_sigma_e_eV=tracker_sigma_e_eV,
    ...
    def __init__(self, mf, *, nstates=5, grid_level=None, conv_tol=None,
        ...
        self.mf = mf
        self.mol = mf.mol
        # Default unit label from the molecule
        try:
            self.unit = 'Angstrom' if str(self.mol.unit).lower().startswith('ang') else 'Bohr'
        except Exception:
            self.unit = 'Angstrom'
        self.nstates = int(nstates)
        self.grid_level = grid_level if grid_level is not None else getattr(mf.grids, 'level', 4)
        ...
        self.geom_ref = None          # atom-list (Å) reference geometry
        self.l_MWC = None             # (3N, Nvib)
        self.evals_int_au = None      # (Nvib,)
        self.mass_vec_3N = None       # (3N,)

        # Outputs / accessible fields:
        self.ref = None               # dict of TDDFT reference results
        self.M0_au = None             # (3,)
        self.M0_D = None              # (3,)
        self.M0_mag_au = None
        self.M0_mag_D = None
        self.omega_elec_au = None
        self.omega_elec_eV = None
        self.f_osc = None
        self.ht = None                # dict of HT derivative results
        self.dmu_dQ_au = None         # (Nvib,3)
        self.dmu_dDelta_au = None     # (Nvib,3)
        self.valid_mask = None        # (Nvib,)
        self.omega_vib_au = None      # (Nvib,)

    # -------------------- Public: Reference TDDFT --------------------
    def compute_reference(self, istate=0):
        """TDDFT at the *current mf geometry*; caches M0 and related info."""
        ...
        # Promote to easy attributes
        self.M0_au = self.ref['M_au']
        self.M0_D  = self.ref['M_D']
        self.M0_mag_au = self.ref['M_mag_au']
        self.M0_mag_D  = self.ref['M_mag_D']
        self.omega_elec_au = self.ref['omega_au']
        self.omega_elec_eV = self.ref['omega_eV']
        self.f_osc = self.ref['f_osc']
        return self.ref

    # -------------------- Public: HT derivatives --------------------
    def compute_ht_derivatives(self, istate=0, dq_Q=0.03, use_richardson=True):
        ...
        Requires: geom_ref (atom-list), l_MWC, evals_int_au, mass_vec_3N to be set.
        """
        ...
        # Ensure reference M0 is available
        ...
        for k in range(Nvib):
            if not pos[k]:
                continue
            lcol = l_CART[:, k]
            R_plus  = self._displace_in_Q(self.geom_ref, lcol, +dq_Q, unit=self.unit)
            R_minus = self._displace_in_Q(self.geom_ref, lcol, -dq_Q, unit=self.unit)
            mu_p, _ = self._td_tdm_match(R_plus,  e0, mu0)
            mu_m, _ = self._td_tdm_match(R_minus, e0, mu0)
            D1 = (mu_p - mu_m) / (2.0 * dq_Q)
            if use_richardson:
                h2 = dq_Q * 0.5
                R_p2 = self._displace_in_Q(self.geom_ref, lcol, +h2, unit=self.unit)
                R_m2 = self._displace_in_Q(self.geom_ref, lcol, -h2, unit=self.unit)
    ...
    @property
    def M_HT_Q(self):
        """Return dμ/dQ_k (a.u. per Q-unit) — shape (Nvib, 3)."""
        return self.dmu_dQ_au

    @property
    def M_HT_Delta(self):
        """Return dμ/dΔ_k (a.u.) — shape (Nvib, 3)."""
    ...
    def _td_tdm_match(self, geom_atomlist_or_array, ref_energy_au, ref_mu_au):
        """SCF+TDDFT at new geometry (clone mf.mol), then choose root that best matches reference."""
    ...
    def _geom_to_array(geom):
        # Accepts either PySCF atom list or (N,3) array-like
    ...
    def _displace_in_Q(self, geom0, l_cart_col, dq_Q, unit='Angstrom'):
        # ΔR_flat (Bohr) = dq_Q * l_CART[:,k]; convert to Å if needed and reshape
